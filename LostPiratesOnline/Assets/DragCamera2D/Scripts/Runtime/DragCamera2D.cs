...
#if UNITY_ANDROID
    float ekranabasilankonumx = 0, ekranabasilankonumy = 0;
#endif
...
;
    ...
    public void kamerakaydırmaHiziAyarla()
    {
        if (GameManager.gm.KameraKaydırmaSlider.value == 1)
        {
            dragSpeed = -0.08f;
            touchDragSpeed = -0.04f;

        }
        else if (GameManager.gm.KameraKaydırmaSlider.value == 2)
        {
            dragSpeed = -0.1f;
            touchDragSpeed = -0.06f;
        }
        else if(GameManager.gm.KameraKaydırmaSlider.value == 3)
        {
            dragSpeed = -0.2f;
            touchDragSpeed = -0.08f;
        }
        else if(GameManager.gm.KameraKaydırmaSlider.value == 4)
        {
            dragSpeed = -0.3f;
            touchDragSpeed = -0.1f;

        }
        else if (GameManager.gm.KameraKaydırmaSlider.value == 5)
        {
            dragSpeed = -0.4f;
            touchDragSpeed = -0.11f;
        }
    }
    ...
    public void panControl()
    {
        ...
        if (!GameManager.gm.FiloAna.activeSelf && !GameManager.gm.Etkinlik.activeSelf && !GameManager.gm.Profil.activeSelf && !GameManager.gm.Market.activeSelf && !GameManager.gm.Tershane.activeSelf && !GameManager.gm.SeyirDefteri.activeSelf && !GameManager.gm.Siralama.activeSelf && !GameManager.gm.Ayarlar.activeSelf && !GameManager.gm.YetenekSistemi.activeSelf && !GameManager.gm.winchatinputfield.isFocused && keyboardInput && (Input.GetKey(KeyCode.W)|| Input.GetKey(KeyCode.S) || Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)))
        {
...
#if UNITY_ANDROID
        // if mouse is down
        if (Input.GetMouseButton((int)mouseButton) && !IsPointerOverUIObject())
        {
            float x = Input.GetAxis("Mouse X") * touchDragSpeed;
            float y = Input.GetAxis("Mouse Y") * touchDragSpeed;

            
            // ekran kaydırılıyorsa
            if (ekranabasilankonumx == 0 && ekranabasilankonumy == 0)
            {
                ekranabasilankonumx = x;
                ekranabasilankonumy = y;
            }
            if (Input.touchCount == 1 && Math.Abs(ekranabasilankonumx - x) > 0.1f || Math.Abs(ekranabasilankonumy - y) > 0.1f)
            {
                //kameranın targeti null olacak
            ...
            if (GameManager.gm.kamerakaydirmajoyistik.gameObject.activeSelf)
            {
                x = GameManager.gm.kamerakaydirmajoyistik.Horizontal;
                y = GameManager.gm.kamerakaydirmajoyistik.Vertical;
            }
            transform.Translate(x, y, 0);
        ...
        {
            ekranabasilankonumx = 0;
            ekranabasilankonumy = 0;
        }
    ...
    private void cameraClamp()
    {
        ...
        if (GameManager.gm.BenimGemim.GetComponent<Player>().harita < 97)
        {
            boundsMaxX = GameManager.gm.kameraLimitleri[GameManager.gm.BenimGemim.GetComponent<Player>().harita + 1].pointa.x;
            boundsMinX = GameManager.gm.kameraLimitleri[GameManager.gm.BenimGemim.GetComponent<Player>().harita + 1].transform.position.x;
            boundsMaxY = GameManager.gm.kameraLimitleri[GameManager.gm.BenimGemim.GetComponent<Player>().harita + 1].pointa.y;
            boundsMinY = GameManager.gm.kameraLimitleri[GameManager.gm.BenimGemim.GetComponent<Player>().harita + 1].transform.position.y;
        }
        else if (GameManager.gm.BenimGemim.GetComponent<Player>().harita == 98 )
        {
            boundsMaxX = GameManager.gm.kameraLimitleri[12].pointa.x;
            boundsMinX = GameManager.gm.kameraLimitleri[12].transform.position.x;
            boundsMaxY = GameManager.gm.kameraLimitleri[12].pointa.y;
            boundsMinY = GameManager.gm.kameraLimitleri[12].transform.position.y;
        }
        else if (GameManager.gm.BenimGemim.GetComponent<Player>().harita == 97)
        {
            boundsMaxX = GameManager.gm.kameraLimitleri[13].pointa.x;
            boundsMinX = GameManager.gm.kameraLimitleri[13].transform.position.x;
            boundsMaxY = GameManager.gm.kameraLimitleri[13].pointa.y;
            boundsMinY = GameManager.gm.kameraLimitleri[13].transform.position.y;
        }
        else
        {
            boundsMaxX = GameManager.gm.kameraLimitleri[0].pointa.x;
            boundsMinX = GameManager.gm.kameraLimitleri[0].transform.position.x;
            boundsMaxY = GameManager.gm.kameraLimitleri[0].pointa.y;
            boundsMinY = GameManager.gm.kameraLimitleri[0].transform.position.y;
        }
